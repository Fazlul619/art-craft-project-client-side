import { Star } from "lucide-react";
import { useLoaderData, useParams } from "react-router-dom";

const ItemDetails = () => {
  const data = useLoaderData();
  const { id } = useParams();

  const currentItem = data.filter((item) => item._id === id);

  return (
    <div className="w-[90vw] mx-auto mt-16">
      {currentItem.map((item) => {
        const stars = [];

        for (let i = 0; i < 5; i++) {
          if (i < item.rating) {
            stars.push(<Star key={i} className="h-4 w-4 fill-yellow-300" />);
          } else {
            stars.push(<Star key={i} className="h-4 w-4" />);
          }
        }

        return (
          <div key={item._id} className="flex gap-8">
            <div className="flex-[1]">
              <img
                src={item.imageUrl}
                alt={item.itemName}
                className="rounded-md h-[350px] w-full"
              />
            </div>

            <div className="flex flex-col gap-4 flex-[2]">
              <div>
                <h1 className="text-3xl font-medium font-serif capitalize">
                  {item.itemName}
                </h1>
                <h5 className="text-xs font-semibold">
                  {item.subcategoryName}
                </h5>
              </div>

              <span className="flex gap-1">{stars}</span>

              <span>{item.price} $</span>

              <p className="text-sm text-justify">{item.shortDescription}</p>

              <p className="text-xl font-bold">
                Customizable ? {item.customization}
              </p>

              <span className="text-sm font-semibold">
                Added on: {""}
                {item.processingTime}
              </span>

              <p>
                Availability:{" "}
                <span
                  className={`font-mono underline underline-offset-2 px-3 py-1 rounded-sm ${
                    item.Stock === "Stock" ? "bg-green-300" : "bg-yellow-200"
                  }`}
                >
                  {item.Stock}
                </span>
              </p>

              <p className="font-bold">
                Uploaded by: {""}
                <span className="font-medium">
                  {item?.name}
                  {item?.email}
                </span>
              </p>
            </div>
          </div>
        );
      })}
    </div>
  );
};

export default ItemDetails;
